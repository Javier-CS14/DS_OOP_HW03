//********************************************
// Student Name			: Javier Evano Cahyadi
// Student ID			: 114550201
// Student Email Address: javierevano.cs14@nycu.edu.tw
//********************************************
//
// Prof. Sai-Keung WONG
// Email:	cswingo@cs.nctu.edu.tw
//			wingo.wong@gmail.com
//
// National Chiao Tung University, Taiwan
// Computer Science
// Date: 2019/04/09
//
//
// Add your own functions if necessary
//
#ifndef __MY_2048_SYSTEM_H_
#define __MY_2048_SYSTEM_H_
#include "../headers.h"
#include <string>
#include "../primitives/vector3d.h"
#include <vector>
#include "base_class.h"
#include "graph_basics.h"

using std::vector;

#define MAX_2048_NX 10
#define MAX_2048_NZ 10


class MY_2048 : public BASE_SYSTEM {
protected:
    void draw_GridCell( int ix, int iz ) const;
    void draw_GridCells( ) const;
    void draw_Board( ) const;
    void draw_Event( ) const;

    void reset( );
    void generateNumber( );
    void moveDown( );
    void moveUp( );
    void moveLeft( );
    void moveRight( );
    //
    void rotate_CounterClockwise( );
    void rotate_Clockwise( );
    void copy( int src[][MAX_2048_NX], int tgt[][MAX_2048_NX]);
    //
    void moveItem( int zIndex, int targetIndex, int i );
    int findItem( int startIndexZ, int i ) const;
    void moveDown_Column( int i );
    //
    void generateNumbers_All_2to4( );
    void generateNumbers_All_2to8( );
    void generateNumbers_All( );

    int countFreeCells( ) const;
    //
    void play_Random( );
    //
    double computeScore( ) const;
    double play_Smart_Recursive( 
        int numSteps, 
        unsigned char action_key,
        int board[][MAX_2048_NX] );
    double play_Smart( int numForwardMoves, unsigned char action_index );
    void play_Smart( int numForwardMoves );

    bool performAction( unsigned char key );
public:
    MY_2048( );
    MY_2048( int nx, int nz );
    void setDimension(int nx, int nz);
    void draw( ) const;
    void askForInput( );
    void handleKeyPressedEvent( unsigned char key );
    void handleSpecialKeyPressedEvent( unsigned char key );
    void setPosition( int x, int z );
    void copy( const BASE_SYSTEM *m );
    MY_2048 &operator=(const MY_2048 &m);
    void setFocus( bool flg );
    void update( );

    bool isAutoPlay() const;

    void copyPrevious( const BASE_SYSTEM *m );


protected:
    ///////////////////////////////////////////
    bool mFlgFocus;
    int mPositionX, mPositionZ;
    unsigned char mEvent;
    ///////////////////////////////////////////
    int mNX, mNZ;
    int mNumFreeCells;
    int mBoard[MAX_2048_NZ][MAX_2048_NX];
    int mTmpBoard[MAX_2048_NZ][MAX_2048_NX];
    //
    int mPreviousBoard[MAX_2048_NZ][MAX_2048_NX];

    //
    bool flg_AutoPlay;
    
};

#endif